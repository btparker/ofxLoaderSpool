...
void ofxLoaderSpool::addBatch(string id){
    LoaderBatch* batch = new LoaderBatch(id);
    batches[id] = batch;
}
...
LoaderBatch* ofxLoaderSpool::getBatch(string id){
    return batches[id];
}
...
ofxLoaderSpool::~ofxLoaderSpool(){
    for(map<string,LoaderBatch*>::iterator iter = batches.begin(); iter != batches.end(); ++iter)
    {
        string k =  iter->first;
        delete batches[k];
    }
